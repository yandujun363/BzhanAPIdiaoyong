...
function wei_key(url_val) {
    ...
    if (cookie_value("gaia_vtoken") !== false) {
        url_val["gaia_vtoken"] = cookie_value("gaia_vtoken")
    }
    ...
    if (mid !="") {
        if (/^[0-9]{1,19}$/g.test(mid) == true) {
            ...
            if (query_video_data.code == 0) {
                ...
                if (query_video_data.code == -352) {
                    lift_risk_control(query_video_data.data.v_voucher)
                } else {
                    ...
                    $.ajax({
                        ...
                        success: function (live_user_data) {
                            if (live_user_data.code == 0) {
                                $.ajax({
                                    ...
                                    success: function (live_room_key_data) {
                                        if (live_room_key_data.code == 0) {
                                            $(".content p").text("加载完成!");
                                            window.location.href="/live-room-barrage.html?"+encodeURI("name="+live_user_data.data.name+"&face="+BASE64.urlsafe_encode(live_user_data.data.face)+"&user_cover="+BASE64.urlsafe_encode(live_room_data.data.user_cover)+"&live_status="+live_room_data.data.live_status+"&live_time="+live_room_data.data.live_time+"&title="+live_room_data.data.title+"&roomid="+live_room_data.data.room_id+"&key="+live_room_key_data.data.token)
                                ...
                                if (live_user_data.code == -352) {
                                    lift_risk_control(live_user_data.data.v_voucher)
                                } else {
...
//===解除风控===
function lift_risk_control(v_voucher) {
    let captcha_key = ""
    $.ajax({
        type: "POST",
        ...
        data: "csrf="+cookie_value("bili_jct")+"&v_voucher="+v_voucher,
        xhrFields: {
            withCredentials: true
        },
        success: function (data) {
            captcha(data.data.geetest.gt,data.data.geetest.challenge)
            setTimeout(function i() {
                if (captcha_key != "") {
                    $.ajax({
                        ...
                        data: "csrf="+cookie_value("bili_jct")+"&challenge="+data.data.geetest.challenge+"&token="+data.data.token+captcha_key,
                        success: function (data) {
                            let cookie_time = new Date(new Date().getTime() + 60 * 1000 * 525600)
                            document.cookie = "gaia_vtoken="+data.data.grisk_id+";Domain=.bilibili.com;Path=/;Expires="+cookie_time
                            document.cookie = "x-bili-gaia-vtoken="+data.data.grisk_id+";Domain=.bilibili.com;Path=/;Expires="+cookie_time
                            alert("解除风控成功，请刷新页面")
                        }
                    });
                } else {
    ...
    function handler (captchaObj) {
        captchaObj.onReady(function(){
            ...
            captcha_key = "&validate="+result.geetest_validate+"&seccode="+result.geetest_seccode
        })
    }
...
function background_image() {
    $.ajax({
        type: "get",
        url: "/json/background_image_Information.json",
        dataType: "json",
        success: function (data) {
            let max = data.length-1
            let i = Math.floor(Math.random() * (max - 0 + 1)) + 0;
            let pid_and_p = data[i].pid_and_p
            ...
            $(".pid_and_p").text("pid&p："+pid_and_p);
            $(".pid_and_p").attr("href",url);
        }
    });
}
//======
...
function url_parameters_to_json() {//url参数转json
    const url_parameter = decodeURIComponent(window.location.search.replace("?","")).split("&")
    if (url_parameter != "") {
        let json = {}
...
//==============================
function User_Information(mid) {
    const User_Information_Data = $.ajax({
        type: "get",
...
function webSocket(){
    if("WebSocket" in window){
        ...
        ws.onopen = function(e){
            console.log("open");
            let live_room_key = url_parameters_to_json()
            delete live_room_key.mid
            delete live_room_key.name
            delete live_room_key.face
            delete live_room_key.user_cover
            for(let i in live_room_key) {
                if (/^\d+$/.test(live_room_key[i]) == true) {
                    live_room_key[i] = live_room_key[i]*1
                }
            }
            var certification = live_room_key
